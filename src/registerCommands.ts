...
import { buildAdapterCommands, buildBotCommands, buildMonitorCommands } from "./commands.js";

const config = loadConfig();
const rest = new REST({ version: "10" }).setToken(config.discordToken);
const body = [...buildMonitorCommands(), ...buildAdapterCommands(), ...buildBotCommands()].map((command) => command.toJSON());
if (body.length > 100) {
