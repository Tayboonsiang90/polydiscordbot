...
export function extractEthereumGasMonthlyValue(data: unknown): string {
  const snapshot = extractEthereumGasMonthlySnapshot(data);
...
export const ethereumGasMonthlyAdapter: WebsiteAdapter = {
  ...
  getPollIntervalReason: () => "Fixed hourly check for finalized Dune monthly Ethereum gas averages",
  async fetchCurrentValue(): Promise<AdapterValue> {
    const apiKey = process.env.DUNE_API_KEY;
    if (!apiKey) {
      throw new Error("Missing DUNE_API_KEY. Add it to .env to monitor Dune Ethereum monthly gas prices.");
    }
    ...
    const value = extractEthereumGasMonthlyValue(await response.json());
    return {
      value,
      rawValue: value,
      unit: "mean_gas",
