...
const expectedCommandIds = [
  ...
  ["bigbrother", "big-brother-episodes"],
  ...
  ["typhoondolphin", "jma-typhoon-dolphin"],
...
describe("adapter registry", () => {
  ...
  it("keeps known dynamic polling and special-command capabilities", () => {
    ...
    expect(getAdapterByCommandName("bigbrother").getPollIntervalMinutes?.({} as never)).toBe(1);
    ...
    expect(getAdapterByCommandName("whpool").fetchEventUpdates).toBeDefined();
    expect(getAdapterByCommandName("whpool").getPollIntervalMinutes?.({} as never)).toBe(1);
    ...
    expect(getAdapterByCommandName("cyclospora").refreshSettings).toBeDefined();
    expect(getAdapterByCommandName("cyclospora").upsertPolymarketMarket).toBeDefined();
