...
| `metadao-credible-fundraise` | `/monitor` | `#metadao-credible` | `MetaDAO Credible Alerts` | `🏛️` | Monitors the Credible Finance MetaDAO public sale hourly using launchpad on-chain data for total committed USDC and contributor count. |
...
- Shared value-change alert UI lives in `src/embeds.ts`; keep the top of alerts human-first (`Quick read`, then compact snapshots), format important timestamps for Discord, and push URLs/raw state into the shared `Links` field or lower-priority snapshot lines.
- UFO Files fingerprints official UFO/UAP file-link inventories from NARA, AARO, and FBI sources; NARA is fetched directly while AARO/FBI use `r.jina.ai` mirrors because direct Node fetches are blocked. Alerts include source-level count/fingerprint changes because the visible sample is not a full diff.
...
- MetaDAO Credible Fundraise polls hourly, reads the Credible launchpad account and funding records from public Solana RPC, and reports total committed USDC plus positive contributor count. Direct `metadao.fi` page HTML currently returns a Vercel challenge to bot HTTP, so the adapter keeps the official page as the resolution link but uses the same launchpad on-chain state for runtime checks.
