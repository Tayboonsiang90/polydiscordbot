...
| `aaa-regular-gas` | `/monitor` | `#aaa-regular-gas` | `AAA Gas Alerts` | `⛽` | Monitors AAA national Current Avg. Regular gas, displays the published value and market-rule two-decimal truncation, and auto-discovers monthly markets. |
...
| `hk-precip` | `/monitor` | `#hkprecip` | `HKO Hong Kong Precip Alerts` | `☔` | Monitors HKO monthly rainfall plus 1-minute RF023 Observatory AWS alpha; the first positive past-hour snapshot in each clock hour alerts, while zero reports are ignored. |
...
| `met-office-london-precip` | `/monitor` | `#londonprecip` | `Met Office London Precip Alerts` | `☔` | Monitors Met Office Heathrow rain plus 1-minute hourly alpha from Environment Agency Heathrow Airport gauge 247540TP; four 15-minute readings form each hour. |
...
| `noaa-seattle-precip` | `/monitor` | `#seattleprecip` | `NOAA Seattle Precip Alerts` | `☔` | Monitors NOAA's official SEAthr Seattle City Area monthly thread plus 1-minute KSEA airport hourly alpha; positive or trace hours alert while zero hours remain silent. |
...
- The five recurring monthly precipitation monitors use the rule location rather than a nearby substitute: HKO Observatory AWS RF023, Central Park KNYC, KMA Seoul station 108, Heathrow Airport gauge 247540TP, and NOAA's SEAthr Seattle City Area thread with current KSEA airport hourly observations. They poll hourly alpha feeds every minute, retain positive/trace observations for the station-local day, and alert only when a positive hour is added or revised; zero hourly reports are ignored. HKO and Heathrow hourly values are explicitly provisional alpha and do not replace their official monthly resolution gauges. London's slower Infoclimat/Weather.com cumulative alpha is cached for 30 minutes so 1-minute hourly polling does not multiply those requests. All five continue checking even before a new Polymarket URL is discovered.
