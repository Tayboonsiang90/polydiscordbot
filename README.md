...
| `box-office-weekends` | `/monitor` | `#boxoffice` | `Box Office Alerts` | `🎬` | Monitors all active Polymarket weekend box-office markets in one channel, sums The Numbers daily domestic weekend rows, includes opening-preview rows when present, watches BoxOfficeMojo as a secondary update signal, and auto-discovers new box-office markets. |
...
- NYT Front Page accepts either the parent weekly Polymarket event URL or a nested outcome URL; the adapter normalizes nested outcome URLs to the parent event before date-window parsing and strike parsing.
...
- ISW Ukraine Map polls the official ArcGIS StoryMap item JSON first, then falls back to the full StoryMaps HTML page, so the 1-minute monitor is less sensitive to large-page fetch timeouts.
...
- Box Office Weekends keeps all active weekend box-office markets in one adapter/channel, discovers active `weekend box office` Polymarket events with the `box-office` tag, reads The Numbers pages through `r.jina.ai` because direct The Numbers bot HTTP returns 403, checks BoxOfficeMojo title/release pages as a secondary movie update signal, keeps prior bondable brackets through transient fetch errors, and alerts only when a market becomes complete/bondable, its complete bracket changes, or BoxOfficeMojo updates after a baseline exists.
