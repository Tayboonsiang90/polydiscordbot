...
| `uma-voting-committee` | `/umarocks` | `#uma-rocks-votes` | `UMA.rocks Alerts` | `🗳️` | Monitors recent UMA.rocks voting committee GitHub rounds every 10 minutes, checks the branch head directly for low-latency answer commits, and catches PR comments or reviews even after merge. |
...
| `white-house-live` | `/monitor` | `#whlive` | `White House Live Alerts` | `📺` | Checks the official White House Live page every minute and alerts when a livestream is scheduled, changes, or goes live; repeated offline states are suppressed. |
...
| `white-house-videos` | `/monitor` | `#whvideos` | `White House Video Alerts` | `🎬` | Checks the official White House Videos page every minute and alerts for every new or revised livestream, short, or standard video with direct White House and YouTube links. |
| `white-house-youtube-streams` | `/monitor` | `#whstreams` | `White House Stream Alerts` | `📺` | Checks the official White House YouTube Streams tab every minute and alerts for new streams, schedule changes, and live-status transitions without monitoring ordinary uploads or Shorts. |
...
| `jma-typhoon-dolphin` | `/monitor` | `#typhoon-dolphin` | `Typhoon Dolphin Alerts` | `🌀` | Polls JMA's official Dolphin center every minute, classifies it against detailed China/Hong Kong/Macau and Japan land polygons, tracks both active markets, and role-alerts only on a first qualifying territory crossing. |
