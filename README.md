...
| `noaa-seattle-precip` | `/monitor` | `#seattleprecip` | `NOAA Seattle Precip Alerts` | `☔` | Monitors NOAA's official SEAthr Seattle City Area monthly thread plus 1-minute KSEA airport hourly alpha; positive rain, traces, and revisions alert while zero-only updates are stored silently. |
...
- NOAA NYC and Seattle monthly precipitation use RCC ACIS daily rows instead of only the monthly sum. NYC retains its existing alerts for newly posted `0.00` or `T` days; Seattle stores `0.00` days silently and alerts for positive rain, `T`, or a changed monthly total.
